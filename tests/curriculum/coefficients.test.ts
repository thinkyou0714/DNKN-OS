...
const GOLDEN_ID_SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ["omega#0", "周波数 f [Hz] が与えられたコイル L の誘導性リアクタンス X_L は？"],
  ["omega#1", "50 Hz の系統の角周波数 ω はおよそいくつか？"],
  ["omega#2", "同期速度 Ns = 120f/p を計算するとき、f を ω=2πf に置き換える必要はあるか？"],
  ["omega#3", "直列共振の角周波数は ω0 = 1/√(LC)。これを共振「周波数」f0 [Hz] に直すと？"],
  ["waveform#0", "正弦波を全波整流したときの平均値は、最大値 Vm の何倍か？"],
  ["waveform#1", "正弦波の波形率（実効値 ÷ 平均値）はいくつか？"],
  ["waveform#2", "可動コイル形計器で整流形交流電圧計を作るとき、目盛はどう作られているか？"],
  ["waveform#3", "振幅 Vm の対称方形波（±Vm を半周期ずつ）の波形率はいくつか？"],
  ["base#0", "三相基準（基準容量 Sb=三相容量・基準電圧 Vb=線間電圧）での基準電流 Ib は？"],
  ["base#1", "三相基準での基準インピーダンス Zb は？"],
  ["base#2", "1相分の容量で %Z が表示された単相変圧器を、3台で三相を組んで三相基準の系統に入れる。%Z の扱いは？"],
  ["base#3", "三相短絡電流を p.u. で求めた。実量[A]に戻すために掛ける値は？"],
];
