...
| **電気設計計算ツールキット（第2の商品・通年需要）** | `web/toolkit.html`＋`lib/toolkit/`（設計実務者向けの独立ページ。**電圧降下(単相2線/直流/単相3線/三相3線・力率＋リアクタンス)・力率改善コンデンサ容量・短絡電流/遮断容量(%Z法)・接地抵抗(A/B/C/D種)・抵抗ディレーティング・コンデンサ寿命(10℃2倍則＋周波数補正)・半導体熱計算(定常＋繰り返しパルス Zth)・IPC-2221パターン幅**の8モジュール（前半4つは電験の論点と直結＝学習アプリと相互送客）。計算結果だけでなく**「なぜその式・係数か」の根拠解説**と**計算書印刷(PDF)**つき。完全ブラウザ内処理=データ非送信。無料3＋有料5モジュールを sku=`toolkit` のライセンスキーでゲート・**公開鍵未設定では全機能無料のまま**） | `docs/strategy/toolkit-product.md` |
| **帳票変更点抽出ツール（第3の商品・B2B/競合最薄）** | `web/sheet-diff.html`＋`lib/sheet-diff/`（部品表・端子表などの設計帳票を**キー列で突合**し、追加/削除/**セル単位の変更**を抽出。行の並べ替えを変更と誤検出しない・**各社雛形の列名エイリアス**・数値表記差(1.0と1)の同値扱い・**Excelブック(.xlsx)の直読み**(ブラウザ内unzip・外部ライブラリなし)・**Shift_JIS自動判定**・キー重複検出。CSV書き出し/レポート印刷/プリセット保存が sku=`sheetdiff` の有料枠。**帳票は外部送信せず localStorage にも保存しない**） | `docs/strategy/sheet-diff-product.md` |
